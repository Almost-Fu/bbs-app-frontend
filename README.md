...
- 足迹、关注的吧：游客显示「登录后查看」；已登录但还没逛过吧 / 没关注吧时显示引导文案（不会误报成「未登录」）
- 我的帖子 / 我的收藏（登录可见）
...
- **自定义头像**：编辑资料页支持「从相册选择 / 拍照」上传自己的图片（`POST /api/users/me/avatar`，multipart 字段名 `file`，上传即写库），也可以直接填写图片地址。上传的图存在后端 `/uploads/avatars/`（免费实例是临时盘，重新部署会丢 → 前端自动回落到默认头像，不会破图）。
- **冷启动提示**：后端是免费云实例（15 分钟无请求会休眠，唤醒需 30~60 秒）。请求变慢时依次提示「正在连接服务器…」→「服务器正在唤醒（免费实例会自动休眠）」→「首次访问需 30~60 秒，请稍候…」；App 启动时还会**静默预热** `/api/health`，请求超时放宽到 **90 秒**，避免冷启动被误判为失败。
- 演示帖的配图用网络占位图（picsum.photos），需联网加载；吧图用 `static/images/bars` 下的本地图（数据库 `bars.image` 里存的就是这些路径，**后端 `/static/images/bars/` 也放了同一份**，管理后台就是从后端加载的）。
  ⚠️ **吧图文件名必须是纯 ASCII**（`bar-1.jpg` … `bar-16.jpg`）：HBuilderX 打 App 包时不允许中文 / 全角文件名，早期中文名（如「养猫.jpg」）会导致打包直接报错中止（H5 没这个限制，所以一直没暴露）。
